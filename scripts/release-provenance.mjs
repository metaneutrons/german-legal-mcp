...
async function sha256File(path) {
  ...
  if (!metadata.isFile() || metadata.isSymbolicLink() || metadata.nlink !== 1) {
    throw new Error(`Provenance subject must be one regular, non-symlinked file: ${basename(path)}`);
  }
...
async function readProvenanceStatement(path) {
  const metadata = await lstat(path);
  if (!metadata.isFile() || metadata.isSymbolicLink() || metadata.nlink !== 1) {
    throw new Error('Release provenance must be one regular, non-symlinked file.');
  }
