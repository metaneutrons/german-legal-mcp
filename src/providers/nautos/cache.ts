import { readFile, mkdir, readdir, lstat, unlink, utimes } from 'fs/promises';
import { createHash } from 'crypto';
import type { TocSection, DocumentDetail } from './client.js';
import { cachePath } from '../../shared/state-paths.js';
import { atomicWriteJson, withFileTransactionLock } from '../../shared/persistence.js';
...
async function readUnlocked(acCode: string, touch: boolean): Promise<CachedDocument | null> {
  try {
    ...
    if (!file.isFile() || file.isSymbolicLink() || file.size > NAUTOS_MAX_DOCUMENT_BYTES) return null;
    const raw = await readFile(path, 'utf-8');
    const value = JSON.parse(raw) as unknown;
    if (!isCachedDocument(value, acCode)) return null;
    if (Date.now() - value.fetchedAt > TTL_MS) return null;
    if (touch) {
      const now = new Date();
