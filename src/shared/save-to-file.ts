...
async function ensureExportRoot(configuredRoot: string): Promise<string> {
  ...
  try {
    ...
    if (process.platform === 'win32') {
      pinned = await lstat(configuredRoot);
    } else {
      ...
      try {
        ...
        if (hasErrorCode(error, 'ELOOP') || hasErrorCode(error, 'ENOTDIR')) {
          const rejected = await lstat(configuredRoot);
          assertSafeExportDirectory(configuredRoot, rejected);
        }
        throw error;
      }
      pinned = await handle.stat();
    }
    assertSafeExportDirectory(configuredRoot, pinned);
    ...
    if (!sameIdentity(pinned, resolvedMetadata) || !sameIdentity(pinned, after)) {
...
async function captureDirectoryIdentities(paths: string[]): Promise<DirectoryIdentity[]> {
  ...
  try {
    for (const path of paths) {
      ...
      assertSafeExportDirectory(path, pinned);
      const selected = await lstat(path);
      assertSafeExportDirectory(path, selected);
      if (!sameIdentity(pinned, selected)) {
        await handle?.close();
        throw new Error(`Export directory changed while it was being validated: ${path}`);
      }
      identities.push({ path, dev: pinned.dev, ino: pinned.ino, handle });
    }
    return identities;
  } catch (error) {
...
export async function saveToFile(savePath: string, content: string, meta?: string): Promise<ToolResult> {
  ...
  try {
    ...
    // open, revalidate the full chain immediately before O_EXCL, and let the
    // exclusive create be the existence check. No content is written until the
    // opened inode is proven to be selected through the same parent chain.
    ...
    try {
      ...
      if (hasErrorCode(error, 'ELOOP')) {
        throw new Error(`save_path must not be a symbolic link: ${savePath}`, {
          cause: error,
        });
      }
